...
save_log_dir = '/home/eduarda/Repos/jupyter_ic/notebooks/CMOS/32nm/NOT/MLP_Regressor/SKL/single_output/gridSearch/tphl/TESTES/T1'
...
# Loading models
tphl_model = joblib.load('/home/eduarda/Repos/jupyter_ic/models/trained/MLP_Regressor/CMOS/32nm/NOT/single_output/grid_search_tuning_test/tphl/TESTES/T1/best_GS_tphl.joblib')

X = X_train
y = y_train['tphl']


model_scaler_y = tphl_model.transformer_
...
print(f"Y tphl Test set loaded: {y_test['tphl'].shape}")

# Testing
print("\nTesting tphl")
y_pred = tphl_model.predict(X_test)

y_test_scaled = model_scaler_y.transform(y_test[["tphl"]]).flatten()
y_pred_scaled = model_scaler_y.transform(y_pred.reshape(-1, 1)).flatten()


# Testing R² score
r2 = r2_score(y_test['tphl'], y_pred)
print(f"R²: {r2}")

# Testing error
rmse_physical = root_mean_squared_error(y_test['tphl'], y_pred)
rmse_ps = rmse_physical * 1e12

mape = mean_absolute_percentage_error(y_test['tphl'], y_pred)

print(f'MLP, RMSE physical (s): {rmse_physical}')
...
plt.title('Residual Plot for tphl MLP Model (Physical Scale)')
plt.xlabel('Predicted Values')
plt.ylabel('Residuals / Errors')
plt.grid(True, alpha=0.3)
plt.savefig("/home/eduarda/Repos/jupyter_ic/notebooks/CMOS/32nm/NOT/MLP_Regressor/SKL/single_output/gridSearch/tphl/TESTES/T1/residuals_normalized"
"/residuals.png")
plt.show()
plt.close()

residuals = y_test['tphl'] - y_pred
...
plt.ylabel('Residuals / Errors')
plt.grid(True, alpha=0.3)
plt.savefig("/home/eduarda/Repos/jupyter_ic/notebooks/CMOS/32nm/NOT/MLP_Regressor/SKL/single_output/gridSearch/tphl/TESTES/T1/residuals_physical"
"/residuals.png")
plt.show()
plt.close()
